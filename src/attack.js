...
async function commitAttack(params) {
    ...
    if (currentActor.data.data.wildcard) {
        ...
        // Wildcards remove the least of the rolls (they have at least 2 dice)
    ...
    diceResultPool.forEach((el) => {
        el.bgColor = (el.type == "wildRoll" ? "background-color: rgb(255,215,0, 0.35);" : el.roll.dice.some((el) => el.results.some((el) => el.exploded == true) == true) ? "background-color : rgb(0, 200, 0, 0.35)" : el.roll.total == 1 ? "background-color : rgb(255, 0, 0, 0.35)" : "");

        let totalParry = currentTarget.data.data.stats.parry.value + currentTarget.data.data.stats.parry.modifier;

        if (el.saved) {
            let result = {};
            if (!(macroSettings.displayOption == 2 && attackSkillName == macroSettings.skillFighting)) {
                result = (el.roll.total + totalMod) >= (attackSkillName == macroSettings.skillShooting ? 4 : totalParry) + 4 ? { display: i18n("swadeMacro.commitAttackChat.raise"), color: "background-color: rgb(0, 0, 255, 0.35)" } :
                    (el.roll.total + totalMod) >= (attackSkillName == macroSettings.skillShooting ? 4 : totalParry) ? { display: i18n("swadeMacro.commitAttackChat.hit"), color: "background-color: rgb(0, 200, 0, 0.35)" } : { display: i18n("swadeMacro.commitAttackChat.miss"), color: "background-color: rgb(255, 0, 0, 0.35)" }
    ...
    if (isValidConditions) {
        let chatMessage = await renderTemplate("modules/swade-macros-simple/templates/macro-combat-flow/chat-commit-attack.html", {
            ...
            difficulty: attackSkillName == macroSettings.skillShooting ? "4" : (macroSettings.displayOption == 1) ? currentTarget.data.data.stats.parry.value + currentTarget.data.data.stats.parry.modifier : 0,
...
//    }
    ...
    for (let i = 0; i < successResultPool.length; i++) {
        ...
        if (isShotgunTemplate) {
            ...
            // Alex Added
            // ----------
            if (attackSkillName == macroSettings.skillFighting){
