...
async function rangedAttackForm() {
    ...
    let rangeWeapons = weapons.filter((el) => el.data.data.range != "0" && el.data.data.range != "" && el.data.data.additionalStats != undefined && el.data.data.additionalStats.isLaserWeapon != undefined && el.data.data.additionalStats.isLaserWeapon.value);
...
async function commitAttack(params) {
    ...
    //SWADE rules for how much ammo is expended per RoF. Trippled for Overcharge
    let rofAmmo = { 1: 3, 2: 15, 3: 30, 4: 60, 5: 120, 6: 150 };
    ...
    if (currentActor.data.data.wildcard) {
        ...
        // Wildcards remove the least of the rolls (they have at least 2 dice) if it is not a 1 skillDie (leave it in for overload)
        let leastIndex = diceResultPool.findIndex((el) => el.roll.total == Math.min(...diceResultPool.map((el) => el.roll.total)));
        if(1 != diceResultPool[leastIndex].roll.total || leastIndex == diceResultPool.length - 1) {
           diceResultPool[leastIndex].saved = 0;
        }
    } else if (1 == diceResultPool.length) {
        // Non-wildcards add an unsaved wild-die to Determine Critical Failure (a 1d6 is enough, no need to explode)
        diceResultPool.push({ type: "wildRoll", roll: new Roll("1d6").roll(), saved: 0 });
    }
    ...
    // Ranged attacks suffer if below minStr (this can be mitigated by Soldier & Brawny with a minStrIncr)
    let minStrIncr = currentActor.data.data.additionalStats.minStrIncr;
    minStrIncr = (undefined == minStrIncr || "" == minStrIncr.value) ? 0 : parseInt(minStrIncr.value);
    ...
    diceResultPool.forEach((el) => {
        el.bgColor = (el.type == "wildRoll"                                                           ? "background-color : rgb(255,215,0, 0.35);" :
                      el.roll.dice.some((el) => el.results.some((el) => el.exploded == true) == true) ? "background-color : rgb(0, 200, 0, 0.35);" :
                      el.roll.total == 1                                                              ? "background-color : rgb(255, 0, 0, 0.35);" :
                                                                                                        ""
                     );
        let isOverload = (el.roll.total == 1) && (el.type != "wildRoll");

        if (el.saved || isOverload) {
            let result = {};

            result =
                     isOverload                      ? { display: "Overload", color: "background-color: rgb(0, 0, 0, 0.65)" } :
                     (el.roll.total + totalMod) >= 8 ? { display: i18n("swadeMacro.commitAttackChat.raise"), color: "background-color: rgb(0, 0, 255, 0.35)" } :
                     (el.roll.total + totalMod) >= 4 ? { display: i18n("swadeMacro.commitAttackChat.hit"), color: "background-color: rgb(0, 200, 0, 0.35)" } :
                                                       { display: i18n("swadeMacro.commitAttackChat.miss"), color: "background-color: rgb(255, 0, 0, 0.35)" }

            if (result.display != i18n("swadeMacro.commitAttackChat.miss") && !isOverload) successResultPool.push(result.display);

            result.total = isOverload ? 1 : el.roll.total + totalMod;
            el.display = result;
        };
    });
    ...
    if (isValidConditions) {
        let chatMessage = await renderTemplate("modules/swade-macros-simple/templates/macro-combat-flow/chat-commit-attack.html", {
            ...
            weaponName: weapon.data.name + ".  [Overcharge Mode]",
...
async function damageResult(params) {
    ...
    if (otherDmgMod.length > 0) {
        damageModPool.push({ mod: "otherMod", title: i18n("swadeMacro.damageResultChat.otherMods"), abilitie: 0, value: parseInt(otherDmgMod[0].value) });
    }
    // --------------------
    // Alex Added ends here
    // --------------------
    ...
    for (let i = 0; i < successResultPool.length; i++) {
        ...
        // Add Raise and Overcharge bonus
        ...
        weaponDamage += " + 1d6 + " + totalDamageMod;
        ...
        diceResultPool.push({ type: "damageRoll", roll: roll, raiseMeleeLite: new Roll("1d6x=").roll(), exploded: roll.dice.some((el) => el.results.some((el) => el.exploded == true) == true) });
    }
    ...
    diceResultPool.forEach((el) => {
        ...
        if (macroSettings.displayOption == 1) {
            ...
            let totalToughness = parseInt(currentTarget.data.data.stats.toughness.value) - parseInt(currentTarget.data.data.stats.toughness.armor) + armorValue;
    ...
    let chatMessage = await renderTemplate("modules/swade-macros-simple/templates/macro-combat-flow/chat-damage-result.html", {
        ...
        targetAlreadyShaken: 0,
        toughnessValue: macroSettings.displayOption == 1 ? currentTarget.data.data.stats.toughness.value : 0,
        bennieUsed: bennieUsed,
        armorTitle: "armor : " + currentTarget.data.data.stats.toughness.armor + "\n" + "cover : " + coverBonus,
        armorValue: currentTarget.data.data.stats.toughness.armor + parseInt(coverBonus),
